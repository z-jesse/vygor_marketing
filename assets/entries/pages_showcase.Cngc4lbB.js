...
(function(){
  ...
  const sections = [
    ...
    { showcase: "SHOWCASE - 03", studio: "Coming<br>Soon", city: "NEW YORK CITY", coordinates: "40.7128°N / 74.0060°W", website: "-------------", image: "https://res.cloudinary.com/drihq534t/image/upload/v1765934418/12_13_23_SuiYoga_-21_web-1365x800_fqufi7.jpg" }
  ];
  ...
  const cycleCount = 2; // for seamless loop
  const logicalHeight = vh * total;
  const spacerMultiplier = 50;
  let layers = [];
  let ticking = false;
  let lastSectionIndex = -1;

  /* ---------- Build layers (original style but cleaned up) ---------- */
  ...
  function render(logicalScroll) {
    ...
    for (let idx = 0; idx < layers.length; idx++) {
      ...
      layers[idx].style.transform = \`translate3d(0, \${translateY}px, 0)\`;
    }
  }
  ...
  function handleSnap() {
    ...
    snapTimeout = setTimeout(() => {
      ...
      const offset = logical % vh;
      if (Math.abs(offset - vh / 2) > vh / 3) return; // only snap if close
